...
We wished to examine differences in DETECT-Tool Item patterns between DETECT screenings that did and did not have a DETECT tool recorded intent to report.

###### Table 2.2: Response Patterns: All Items, Positive Intent to Report (#sec2-t2-rp-intent-pos-resp)

We examined responses with a DETECT recorded intent to report.

```{r}
#| label: sec2-rp-intent-pos-resp-subset
# Take response-level subset for the following analyses
t_df <- ms_records |>
  ## Drop "Duplicate" flagged records
  dplyr::filter(!dup_drop) |>
  ## Subset to records with a screening
  dplyr::filter(dt_screened) |>
  ## Subset to records with a DETECT tool recorded intent to report
...
We calculated our response patterns for all DETECT Items, across all screenings with a DETECT recorded intent to report. We stored this internally as Table 2.2.

```{r}
#| label: sec2-t2-rp-intent-pos-resp-table
# Extract response pattern table
## Get table
resp_pattern_pos <- get_dt_resp_pattern(t_df, dt_item_list)
...
###### Table 2.3: Response Patterns: All Items, Positive Intent to Report (#sec2-t3-rp-intent-neg-resp)

We examined responses without a DETECT recorded intent to report.

```{r}
#| label: sec2-rp-intent-neg-resp-subset
# Take response-level subset for the following analyses
t_df <- ms_records |>
  ## Drop "Duplicate" flagged records
  dplyr::filter(!dup_drop) |>
  ## Subset to records with a screening
  dplyr::filter(dt_screened) |>
  ## Subset to records without a DETECT tool recorded intent to report
  dplyr::filter(!dt_aps_reported)
...
We calculated our response patterns for all DETECT Items, across all screenings without a DETECT recorded intent to report. We stored this internally as Table 2.3.

```{r}
#| label: sec2-t3-rp-intent-neg-resp-table
# Extract response pattern table
## Get table
resp_pattern_neg <- get_dt_resp_pattern(t_df, dt_item_list)
