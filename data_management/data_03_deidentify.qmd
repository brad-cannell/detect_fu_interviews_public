...
### "clutter_scale_import.RDS"

We loaded our MedStar follow-up interview clutter scale data.

```{r}
#| label: load-ms-clutter
# Load MedStar Clutter Scale Data
## Path to data
path <- here::here(
    "data", "cleaned_rds_files",
    "clutter_scale_import.rds"
...
## Load
informative_df_import(
    "ms_clutter", path, overwrite = T
  )

 # 2025-07-31: CLUTTER SCALE data imported with 892 rows and 29 columns.
 # Data last modified on OneDrive: 2025-03-27 09:07:10 
```
...
We processed our Clutter Scale Data Set.
...
```{r}
#| label: process-ms-clutter-proxy
# Apply Proxy Map (id, id_ms, ms_resp_num, ms_record_num)
ms_clutter <- ms_clutter |>
  dplyr::mutate(
    id = NA_integer_,
    id_ms = NA_integer_,
    ms_resp_num = NA_integer_,
  ...
  dplyr::relocate(
    'x_primary_key', 'id', 'id_ms', 'ms_resp_num', 'ms_record_num'
    )
```
...
```{r}
#| label: process-ms-clutter-reduce
# Reduce to non-PHI Columns
ms_clutter <- ms_clutter |>
  dplyr::select(
    -c('name_full', 'medstar_id', 'xc_case_id', 'name_last')
  )
```
...
```{r}
#| label: save-medstar-clutter
## Path
path <- here::here(
  "data", "deidentified", "medstar", "08_medstar_clutter_scale"
  )

## Export RDS
saveRDS(ms_clutter, paste0(path, ".rds"))

## Export CSV 
readr::write_csv(ms_clutter, paste0(path, ".csv"))
```
