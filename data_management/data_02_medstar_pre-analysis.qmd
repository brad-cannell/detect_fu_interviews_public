...
There were several entries that remained in our data set for "Et3" testing records or blank records. We removed these from our data set prior to analysis.
...
ms_resps <- ms_resps |>
  dplyr::mutate(
    drop_flag = (
      stringr::str_detect(stringr::str_to_lower(name_full), "et3") |
        is.na(incident_timestamp)
      )
    ) |>
...
##### Comment-Box Patterns

We additionally flagged values for "APS Report Number" that indicated patterns in how Medics utilized the field as a comment box.

```{r}
#| label: clean-dt-comment-box
# Flag patterns of use of the "APS Report Number" field as a comment box"
## Add flags using regular expressions
ms_resps <- ms_resps |>
    dplyr::mutate(
      ### Flag if there is a likely Report Number
      dt_comment_report_num = ifelse(
        stringr::str_detect(dt_aps_report_num,'([0-9]{5,})') |
          stringr::str_detect(
            stringr::str_to_lower(dt_aps_report_num),
            paste0(
              '^([0-9]{3}[ -][0-9]{3}[ -][0-9]{2}|',
               '[0-9]{3}[ -][0-9]{2}[ -][0-9]{2}|',
          ...
          (
            stringr::str_detect(
              stringr::str_to_lower(dt_aps_report_num),
              paste0(
                '(confirmation num|report num|[0-9]+ online report|',
                'report confirmation|e report|[0-9]+[a-z]+ \\(care)'
                )
              ) &
              !stringr::str_detect(
                stringr::str_to_lower(dt_aps_report_num),
                paste0(
                  '(unable to provide report num|report number already filed',
                  'fwpd made)'
                  )
                )
            ) |
          (
            stringr::str_detect(
              ...
              !stringr::str_detect(
                stringr::str_to_lower(dt_aps_report_num),
                paste0(
                  '^(deceased|dos|homeless|l|none|na|pending|tbd|',
                  'transport|vicky)$'
                  )
                )
            ) |
      ...
      ### Flag if there's an APS staff member name or ID
      dt_comment_aps_staffer = ifelse(
        stringr::str_detect(dt_aps_report_num, '[0-9]+') &
          (
            stringr::str_detect(
              stringr::str_remove_all(
                stringr::str_to_lower(dt_aps_report_num), 
            ...
            !stringr::str_detect(
              stringr::str_to_lower(dt_aps_report_num),
              paste0(
                '(e-report confirmation number|care giver was contacted|',
                'on scene for prev|opened by daughter|ily members called 911|',
                'fort worth pd|forth worth pd|vitas on scene|2:27:1|',
                'lack of attention to his medical needs|friend relayed)'
                )
              ) &
            !stringr::str_detect(
              stringr::str_trim(
      ...
      ### Flag if there's a mention that APS was on scene
      dt_comment_aps_onscene = ifelse(
        stringr::str_detect(dt_aps_report_num, '[^0-9#]') &
          (
            stringr::str_detect(
              stringr::str_to_lower(dt_aps_report_num), 
              '(on scene|on-scene| os|was there)'
              ) & !stringr::str_detect(
                stringr::str_to_lower(dt_aps_report_num),
                '(np on scene states|pd on scene and|vitas on scene)'
                )
            ),
        T,
        F
        ),
      ### Flag if there's a mention that APS called EMS
      dt_comment_aps_called_ems = ifelse(
        stringr::str_detect(stringr::str_to_lower(dt_aps_report_num), 'aps') &
          (
            stringr::str_detect(
              stringr::str_to_lower(dt_aps_report_num), 
              paste0(
                '(directed|called|contacted 911|contacted ems|',
                'summoned ems|pt transport)'
                )
              ) & !(stringr::str_detect(
                stringr::str_to_lower(dt_aps_report_num), 
                paste0(
                  '(family members called 911|made and called in talked to',
                  '|was called and)'
                  )
                )
                )
      ...
      ### Flag if there's a mention of someone else making a report
      dt_comment_aps_other_reporter = ifelse(
        !(
          dt_comment_report_num|dt_comment_aps_staffer|dt_comment_aps_onscene|
            dt_comment_aps_called_ems
          ) &
          stringr::str_detect(
            stringr::str_to_lower(dt_aps_report_num),
            paste0(
              '(np on scene|fd reported to aps|fd to file|fwfd follow up|',
              'haltom city fire and jps contacting|fwpd made report|',
              'np on scene states|case opened by dau|rn at facility states|',
              'processed per ccp|pd on scene and report|',
              'stated she is currently working with aps|',
              'to rn who will investigate|reported to mih|river oaks reported|',
              'rn at baylor advises|hospital staff also states they will|',
              'states that will be photographing the scene|',
              'already placed by home health)'
              )
            ),
        T,
        F
      ),
      ### Flag if mention of previous report filed by MedStar
      dt_comment_aps_prev_ems_report = ifelse(
        !(
          dt_comment_report_num|dt_comment_aps_staffer|dt_comment_aps_onscene|
          dt_comment_aps_called_ems
          ) &
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          paste0(
            '(medstar has written|was reported|aps has been notified tw|',
            'reported to aps earlier|made by another crew)'
            )
        ),
        T,
        F
      ),
      ### Flag if there's a mention of an already open APS case
      dt_comment_aps_aware = ifelse(
        (!(
          dt_comment_report_num|dt_comment_aps_staffer|dt_comment_aps_onscene|
            dt_comment_aps_called_ems|dt_comment_aps_prev_ems_report
          ) & (
            stringr::str_detect(
              stringr::str_to_lower(dt_aps_report_num), 
              paste0(
                '(case in progress|case opened by dau|case worker spoken to|',
                'aps home in first|aps is on the ca|filled and being process|',
                'aps was called|open aps case|involved with aps representative',
                "|there's an open case|multiple aps reports pending|",
                'reported that aps had been out|active aps open|active case|',
                'aps case already|already has a active|aps home in the fi|',
                'aps is already taking ca|aps is already work|',
                'aps already has a ca|reports already made and case|',
                'case is in progress|active aps case|pt already has open case',
                '|already on aps rad|already an aps case open|',
                'open report already exist|previously opened case|',
                '[0-9]+ on file)'
                )
              ) & !stringr::str_detect(
                stringr::str_to_lower(dt_aps_report_num),
                paste0(
                  '(plan has been formulated|ly is in the active process of',
                  '|actively seeking|actively working with home)'
                  )
                )
            )
        ) |
        (
          dt_comment_aps_onscene|dt_comment_aps_called_ems|
            dt_comment_aps_other_reporter|dt_comment_aps_prev_ems_report
          ),
        T,
        F
      ),
      ### Flag if mention that patient was deceased
      dt_comment_pt_deceased = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num), 
          '(dos|doa|deceased|obvious death|pt passed away)'
          ),
        T,
        F
      ),
      ### Flag if medic indicated unsure if reporting
      dt_comment_undecided = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
      ...
      ### Flag if medic indicated they were unable to report
      dt_comment_aps_unable = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          paste0(
            "(attempted multiple|answer|disconnected|wouldn't let me|blocked|",
            'office was closed)'
            )
        ),
        T,
      ...
      ### Flag if medic indicated comments in narrative
      dt_comment_narrative = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          '(narrative|narative)'
        ),
      ...
      ### Flag if medic commented that they were not allowed in the patient
      ### home, or gave exigent circumstances for lack of assessment
      dt_comment_did_not_enter = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          paste0(
            '(evicted|on fire|pt would not let ems or fwfd in the house|',
            'homeless|in assisted living|did not view inside|',
            "not the pt's home|not the pt apart|visiting her neighbor)"
            )
        ),
        T,
        F
      ),
      ### Flag if DETECT tool was limited or not done due to patient
      ### condition (or being canceled by PD)
      dt_comment_uta = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          paste0(
            '(no known demographics|pt unresponsive|limited by critical pt|',
            'canceled by fw)'
            )
        ),
        T,
      ...
      ### Flag if medic stated responses were due to the current medical 
      ### condition
      dt_comment_condition = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          '(current medical|current compl|part of medstar program)'
          ),
          T,
          F
      ),
      ### Flag if medic referenced a non-APS referral or outside assistance
      ### they believed was handling the situation
      dt_comment_other_ref = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          paste0(
          '(refer to mih|actively working with home health|',
          'existing caseworker|ongoing investigation by fw|',
          'direction of social worker)'
          )
        ),
        T,
        F
      ),
      ### Flag if medic referenced they believed the situation was being
      ### handled by family or other resources already in place
      dt_comment_resolving = ifelse(
        stringr::str_detect(
          stringr::str_to_lower(dt_aps_report_num),
          paste0(
            '(medical needs|ongoing issues with delusional|',
            'seems to understand what is going on|appointment with medi|',
            'family has already secured|process of establishing a solution|',
            'family members called|patient actively seeking|',
            'known to ems hord|depressed|esolution in process|',
            'vitas on scene)'
          )
        ),
        T,
...
### 2025-03-25 Updates to Reporter Type Definitions

We revised the reporter type classifications in our APS Intake Map data, based on PI feedback (2025-03-25 meeting).

```{r}
#| label: revise-aps-intake-map-reporter-type-classif
# Revise APS Intake Map Classifications
aps_intake_map <- aps_intake_map |>
  ## Revise Definition of "Possible/Reasonable EMS Reporter Types"
  dplyr::mutate(
    reporter_possible = ifelse(
      reporter_type %in% c(
        "Health Care Providers/Staff", "Other Providers"
        ),
      T, 
      F
    )
  ) |>
  ## Create overall classification column
  dplyr::mutate(
    reporter_classif = "any",
    reporter_classif = dplyr::case_when(
      reporter_possible ~ "possible",
      reporter_ems ~ "ems",
      TRUE ~ reporter_classif
    )
  )

# Save revised map file
## Path
path <- here::here(
    "data", "cleaned_rds_files", "record_linkage", "maps", 
    "06_medstar-records_to_aps-intakes_revised.rds"
    )
## Save file
saveRDS(aps_intake_map, path)
```

We also conveyed the reporter type classification updates from our APS Intake Map data to our APS Investigation Map data, based on PI feedback (2025-03-25 meeting).

```{r}
#| label: revise-aps-inv-map-reporter-type-classif
# Convey Intake Updates to APS Investigation Map
## Update Intake Classifications in Investigation Map
aps_inv_map <- dplyr::rows_update(
  aps_inv_map |>
    ### Convert TRUE/FALSE flags for matched intakes to counts (0 as default)
    dplyr::mutate(
      intake_paired = 0,
      intake_ems_poss = 0,
      intake_ems_expl = 0
  ...
  aps_intake_map |>
    ### Reduce to matching key columns and reporter type flag columns
    dplyr::select(id, ems_num, case_id, reporter_possible, reporter_ems) |>
    ### Rename variables to match with the APS Investigation Map
    dplyr::rename_at(
      c("reporter_possible", "reporter_ems"), 
      ~c("intake_ems_poss", "intake_ems_expl")
      ) |>
    ### For each matched APS Case...
    dplyr::group_by(id, ems_num, case_id) |>
    #### Obtain count of paired Intakes, and count with each"reporter class"
    dplyr::mutate(
      intake_paired = max(dplyr::row_number()),
      intake_ems_poss = sum(intake_ems_poss),
      intake_ems_expl = sum(intake_ems_expl)
      ) |>
...
path <- here::here(
    "data", "cleaned_rds_files", "record_linkage", "maps", 
    "07_medstar-records_to_aps-case-invs_revised.rds"
    )
## Save file
saveRDS(aps_inv_map, path)
```
...
intake_df <- aps_intake_map |>
  ...
  dplyr::distinct() |>
  ### Interpret reporter type category from reporter type flag pattern
  dplyr::mutate(
    aps_intake_type = dplyr::case_when(
      aps_intake_ems > 0 ~ "ems",
      aps_intake_poss > 0 ~ "possible",
      aps_intake & aps_intake_ems == 0 & aps_intake_poss == 0 ~ "any",
      TRUE ~ NA_character_
    )
  )
...
temp_df <- ms_resps |>
  ...
  ### Interpret reporter type category from boolean/logical flag pattern
    aps_intake_type = dplyr::case_when(
      aps_intake_ems ~ "ems",
      aps_intake_poss ~ "possible",
      aps_intake & !aps_intake_ems & !aps_intake_poss ~ "any",
      TRUE ~ NA_character_
    )
  ) |>
  ### Reduce to unique rows
