...
We added APS outcomes data. This included the presence of a matched APS Intake (and reporter type categorization) and presence of a matched APS Invesgitation (and its overall disposition).
...
# Add APS Investigation Matching Data to MedStar Response Records
...
## Obtain DF of ID - EMS_NUM - EXACT INV? & DISPO - BROAD INV? & DISPO
best_invs <- dplyr::left_join(
  ### Broader window-based matches (response within 30 days of APS Inv.)
  aps_inv_map |>
    dplyr::filter(best_window) |>
    dplyr::select(id, ems_num, inv_dispo) |>
    dplyr::mutate(aps_inv_window = T) |>
    dplyr::rename_at('inv_dispo', ~'aps_inv_window_dispo'),
  ### Narrower direct-overlap (response during open APS Inv.)
  aps_inv_map |>
    dplyr::filter(best_exact) |>
    dplyr::select(id, ems_num, inv_dispo) |>
    dplyr::mutate(aps_inv_exact = T) |>
    dplyr::rename_at('inv_dispo', ~'aps_inv_exact_dispo'),
  by = c('id', 'ems_num')
  )
...
intake_df <- aps_intake_map |>
  ...
  dplyr::group_by(key) |>
  dplyr::mutate(
    aps_intake_poss =  sum(reporter_possible) > 0,
    aps_intake_ems = sum(reporter_ems) > 0
  ) |>
  dplyr::ungroup() |>
...
ms_subjs <- ms_subjs |>
  dplyr::mutate(
    aps_inv_exact = NA, aps_inv_window = F,
    aps_inv_exact_dispo = NA_character_,
    aps_inv_window_dispo = NA_character_,
    aps_intake = NA, aps_intake_poss = NA, aps_intake_ems = NA,
    aps_intake_type = NA_character_
  )
...
temp_df <- ms_resps |>
  ...
  dplyr::select(
    id, aps_inv_exact, aps_inv_window, aps_inv_exact_dispo, 
    aps_inv_window_dispo, aps_intake, aps_intake_poss, aps_intake_ems
    ) |>
  ...
  dplyr::mutate(
    window_dispos = paste(sort(unique(aps_inv_window_dispo)), collapse = ', '),
    exact_dispos = paste(sort(unique(aps_inv_exact_dispo)), collapse = ', ')
    ) |>
  dplyr::ungroup() |>
  dplyr::mutate(
    ### If there is no "broad match" intake, make exact match NA
    aps_inv_exact = na_if(aps_inv_window, F),
    ...
    aps_inv_exact_dispo = dplyr::case_when(
      stringr::str_detect(exact_dispos, 'invalid') & 
        stringr::str_detect(exact_dispos, ', valid') ~ 
        'Mix of Valid and Invalid',
      stringr::str_detect(exact_dispos, 'invalid') ~ "Invalid",
      stringr::str_detect(exact_dispos, 'valid') ~ 'Valid',
      stringr::str_detect(exact_dispos, 'uta') ~ "Unable to Assess",
      stringr::str_detect(exact_dispos, 'other') ~ "Other (Not Investigated)",
      TRUE ~ NA_character_
    ),
    aps_inv_window_dispo = dplyr::case_when(
      stringr::str_detect(window_dispos, 'invalid') & 
        stringr::str_detect(window_dispos, ', valid') ~ 
        'Mix of Valid and Invalid',
      stringr::str_detect(window_dispos, 'invalid') ~ "Invalid",
      stringr::str_detect(window_dispos, 'valid') ~ 'Valid',
      stringr::str_detect(window_dispos, 'uta') ~ "Unable to Assess",
      stringr::str_detect(window_dispos, 'other') ~ "Other (Not Investigated)",
      TRUE ~ NA_character_
    ),
    ### Interpret reporter type category from boolean/logical flag pattern
    aps_intake_type = dplyr::case_when(
  ...
  dplyr::select(-c(window_dispos, exact_dispos)) |>
  ### Reduce to unique rows
  dplyr::distinct()
